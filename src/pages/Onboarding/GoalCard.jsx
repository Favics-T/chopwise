        ...
        <div
          className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all ${
            isSelected
              ...
              : 'bg-white/80 backdrop-blur-md text-primary shadow-sm'
          }`}
        >
          <Icon size={18} />
        </div>
      ...
      {isSelected && (
        <div className="absolute top-4 right-4 text-primary">
          <CheckCircle2
            size={22}
            fill="currentColor"
            className="text-primary fill-primary bg-white rounded-full"
          />
        </div>
      )}
      <h4 className="font-display text-xl font-bold text-on-surface mb-1 group-hover:text-primary transition-colors">
        {title}
      </h4>
